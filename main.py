...
if __name__ == "__main__":
    ...
    multiObjVec = Mean_vector(100, OBJETIVE)
    ...
    with tf.Session() as sess:
        ...
        if config.learn_mode:
            ...
            try:
                omigas, objectIdex = multiObjVec.get_similar_vectors()
                for objectN in range(len(omigas)):
                    starTime = time.time()
                    #若处理单目标，则随机初始化参数，否则随机选择最近邻居
                    if objectIdex[objectN] == 0:
                        # 从已有的模型中选择最近的邻居，加载其参数
                        rObjModel = "_".join(str(j) for j in omigas[objectN-1])
                        print("restore model from: ", rObjModel)
                        saver.restore(sess, "{}/".format(config.save_to) + rObjModel + "/model.ckpt")

                    objectiveModel = "_".join(str(i) for i in omigas[objectN])
                    with open(config.save_to + "duration.txt", "a+") as f:
                        f.write(objectiveModel + ':')
            ...
            for batchN in range(config.batch_size):
                ...
                # HV
                #hv[batchN] = mobj.get_hyperVolume([550, 100])  # 当vnf = 6 8 10
                hv[batchN] = mobj.get_hyperVolume([600, 100]) #当0805第二次运行Small时，全设置为600
